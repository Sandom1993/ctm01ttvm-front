            ...
            <h-layout-content style="overflow: auto;">
                <div ref="pageBox" class="right-box">
                    <div v-if="isClick" class="form-box">
                        ...
                        >
                            ...
                            <el-form-item v-if="terminalFormMsg.nightLimit" label="夜间限速时段" prop="nightTime">
                                <el-row>
                                    <el-col :span="12">
                                        <el-time-picker
                                            disabled
                                            v-model="terminalFormMsg.startTime"
                                            placeholder="开始时间"
                                            prop="startTime"
                                            clearable
                                            style="width: 235px"
                                    ...
                                    <el-col :span="12">
                                        <el-time-picker
                                            disabled
                                            v-model="terminalFormMsg.endTime"
                                            placeholder="结束时间"
                                            prop="endTime"
                                            clearable
                                            style="width: 235px"
                ...
                >
                    ...
                    >
                        ...
                        <el-form-item v-if="terminalForm.nightLimit" label="夜间限速时段" prop="nightTime">
                            <el-row>
                                <el-col :span="12">
                                    <el-time-picker
                                        v-model="terminalForm.startTime"
                                        placeholder="开始时间"
                                        prop="startTime"
                                        clearable
                                        style="width: 175px"
            ...
            this.$refs.terminalForm.validate((valid) => {
                if (valid) {
                    const params = {
                        ...
                        beginTime: toTimezoneString(this.terminalForm.startTime),
                        endTime: toTimezoneString(this.terminalForm.endTime),
                    };
                    console.log(params)
                    this.loading = true;
                ...
                getDeviceRemoteConfig(params).then(res => {
                    if (res.code === '0' && res.data !== null) {
                        ...
                        if (res.data.beginTime !== "" && res.data.endTime !== "") {
                            // 新增夜间禁行时段字段返回 04:05:00
                            const beginTime = res.data.beginTime;
                            const endTime = res.data.endTime;
                            // const beginTime = "04:05:00";
                            this.terminalFormMsg.startTime = new Date(
                                now.getFullYear(),
                                now.getMonth() +1,
                                now.getDate(),
                                parseInt(beginTime.substring(0, 2)),
                                parseInt(beginTime.substring(5, 3)),
                                parseInt(beginTime.substring(beginTime.length -2, beginTime.length))
                            );

                            this.terminalFormMsg.endTime = new Date (
                                now.getFullYear(),
                                now.getMonth() +1,
                                now.getDate(),
                                parseInt(endTime.substring(0, 2)),
                                parseInt(endTime.substring(5, 3)),
                                parseInt(endTime.substring(endTime.length -2, endTime.length))
                            );
                        }
