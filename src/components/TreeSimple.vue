<template>
  <div class="wrap" :style="'top:'+(top)+ 'px;bottom:'+(bottom)+'px;left:'+(left)+'px;right:'+(right)+'px'">
    <div v-show="isShowResult" class="search-result-wrap">
      <div class="back-to-res">
        <el-button type="iconButton" icon="h-icon-arrow_left" @click="goBackTree">
          {{ $t('GoBack') }}
        </el-button>
      </div>
      <div v-if="queryList.length === 0" class="no-result-wrap">
        <div>
          ...
          <div :style="'margin-top:8px;color: rgba(0,0,0,0.70);font-size:12px;width:'+ (isNeedSearchType ? 318 : 268 )+'px;'">
            查询无结果
          </div>
          <div :style="'margin-top:8px;color: rgba(0,0,0,0.50); font-size:12px;width:'+ (isNeedSearchType ? 318 : 268 )+'px;'">
            请修改条件重新查询
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          <span :title="item.value" style="white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">{{ item.value }}</span>
        </li>
      </el-scrollbar>
    </div>
    <div v-show="!isShowResult" class="tree-wrap">
      ...
      >
        <span style="width:120px;">
          <el-select v-model="searchType" placeholder="请选择">
            <el-option label="SIM卡号" value="0"></el-option>
            <el-option label="责任驾驶员" value="1"></el-option>
            <el-option label="车牌号码" value="2"></el-option>
        ...
        <span style="position: absolute;left:96px;right:0px;">
          <el-autocomplete
            ...
            @select="handleSelect"
          >
            <i slot="suffix" class="el-input__icon h-icon-search" @click="handleIconClick"></i>
          </el-autocomplete>
        </span>
      </div>
      <el-autocomplete
      ...
      <div v-if="isNeedOnline" class="online-device">
        <el-switch
          v-model="isOnline"
          active-color="#2080f7"
          :instruction="treeType==='3' ? $t('ShowOnlineVehicleOnly') : $t('ShowOnlineDeviceOnly') "
          @change="getOnline"
        >
        </el-switch>
      </div>
      <div
        ...
        :style="'top:'+(isNeedOnline ? 72:(isNeedFilter ? 32 : 0))+ 'px;bottom:'+slotLine * 40+'px'"
      >
        <el-tree
          v-if="isShowTree"
          ref="resourceTree"
...
import { getNewOrgTree, getDeviceTree, getVehicleTree, getUserTree, getVehicleCameraTree, getOrgTree,
  filterOrgByName, filterDeviceByName, filterVehicleByName, filterUserTree,
  getTreePath } from '../api/tree.js';

export default {
  name: 'tree-simple',
  props: {
    // 树的类型 1-管理角色组织树 2-设备树 3-车辆树 4-用户树 5-监控点树 6-应用角色组织树
  ...
  methods: {
    ...
    filterFunc(name, type, cb) {
      ...
      if (this.treeType === '1' || this.treeType === '6') {
        p1 = filterOrgByName(Object.assign({
        ...
        p1 = filterVehicleByName(Object.assign({
          searchType: (type === 2 || this.searchType === '车牌号码') ? '2' : this.searchType,
        }, condition));
      ...
      p1.then((json) => {
        if (json.code === '0') {
          ...
          if (typeof cb !== 'undefined') {
            ...
            cb(result.length === 0 ? [{ value: this.$t('HaveNoData') }] : result);
          } else {
    ...
    handleIconClick() {
      const input = document.getElementsByClassName('el-autocomplete-suggestion');
    ...
    checkVehicleNumber(checkedNode) {
      const deviceNum = this.isOnline ? checkedNode.onLineDeviceNum : checkedNode.deviceNum;
      if (deviceNum && parseInt(deviceNum, 10) > this.checkLimit) {
        ...
        this.$message.warning(`为保障性能,建议一次性勾选不超过${this.checkLimit}个${type}`);
    ...
    expandCamera(node, data) {
      if (this.treeType === '5') {
        const arr = data.filter(item => item.children && item.children.length > 0);
        if (node.level !== 0 && arr.length > 0) {
          arr.forEach((item) => {
            setTimeout(() => { // setTimeOut为了规避HUI的缺陷
    ...
    expandDefault() {
      if (this.firstLoad && this.selectOneNode !== '') { // [轨迹回放]默认需要勾选,继续展开
        this.$nextTick(() => {
    ...
    expandPath(node, expandThis) {
      const index = !expandThis ? 2 : 1;
      if (this.treePath.indexOf(node.data.id) !== this.treePath.length - index) { // 还没完全展开
        this.$refs.resourceTree.expandNode(this.treePath[this.treePath.indexOf(node.data.id) + 1]);
      } else if (this.treePath.indexOf(node.data.id) === this.treePath.length - index) { // 已经完全展开
        this.selectNodes(this.selectedResult ? this.selectedResult.id : this.selectOneNode);
        this.getTreeSelected();
    ...
    getTreeSelected() {
      this.$nextTick(() => {
        if (this.isNeedCheckBox) { // 有checkbox
          const checked = this.$refs.resourceTree.getCheckedNodes();
          const children = checked.filter(item => item.isLeaf);
          const parent = checked.filter(item => !item.isLeaf);
          this.$emit('getSelectedNodes', { children, parent });
    ...
    getTreePathArr(id, sourceTypeC, searchResourceTypeC, expandThis = true) { // 车辆-vehicle 设备-ENCODE_DEVICE 驾驶员-driver 组织树-ORG 用户树-
      this.isExpandPath = true;
      let sourceType = 'vehicle';
      if (sourceTypeC) {
        sourceType = sourceTypeC;
      } else if (this.treeType === '1' || this.treeType === '6') {
        sourceType = 'ORG';
      } else if (this.treeType === '2') {
      ...
      const param = {
        indexCode: id,
        sourceType,
        searchResourceType: searchResourceTypeC || this.searchResourceType,
      };
    ...
    loadChildren(node, resolve) {
      if (this.treeType === '5' && node.level !== 0 && node.data.children && node.data.children.length > 0) {
      ...
      p2.then((json) => {
        if (json.code === '0') {
          ...
          this.$nextTick(() => {
            if (this.isExpandPath) { // 是否展开节点的父节点们
              this.expandPath(node, true);
            } else {
              this.getTreeSelected();
            }
          });
    ...
    scrollCurrent(id) {
      setTimeout(() => {
        const nodeArr = Array.from(document.getElementsByClassName('el-tree-node'));
        const nodeCurrent = nodeArr.filter(item => item.getAttribute('data-key') === id);
    ...
    checkNode(id, sourceTypeC, searchResourceTypeC) {
      if (this.isNeedCheckBox) {
        const nodes = this.$refs.resourceTree.getCheckedKeys();
        nodes.forEach((item) => {
          this.$refs.resourceTree.setChecked(item, false);
        });
      } else {
        const key = this.$refs.resourceTree.getSelectedKey();
        this.$refs.resourceTree.setSelected(key, false);
      }
      this.selectedNode = [];
      this.selectedResult = { id };
      this.getTreePathArr(id, sourceTypeC, searchResourceTypeC, false);
    },
  },
};
</script>
...
.rm-tree-device, .rm-tree-device-off{
  font-size:20px
}
.rm-tree-vehicle, .rm-tree-vehicle-off{
  font-size:20px
}
.rm-tree-device-off, .rm-tree-vehicle-off{
  color:#ccc;
}
.rm-tree-user{
  font-size:20px
