...
export default {
  ...
  methods: {
    ...
    handleExpandNode(obj, node, nodeObj) {
      // this.rootIndexCode.push(obj.id);
      // this.isExpandAll = false;
      // this.isExpandPath = false;
      // console.log(this.rootIndexCode)
    },
    handleCollapseNode(obj) {
      // remove(this.rootIndexCode, function (item) {
      //   return item === obj.id
      // })
    },
    // 关闭节点
    ...
    handleReload() {
      // const allNodeList = this.$refs.resourceTree.getCheckedNodes(false, false);
      const leafList = this.$refs.resourceTree.getCheckedNodes(true);
      const parentList = [];
      leafList.forEach(item => {
        parentList.push(item.parentId);
      });
      // this.rootIndexCode = [...parentList]
      const arr = this.$refs.resourceTree.getCheckedKeys();
      this.checkedList = [...arr];
      parentList.forEach(item => {
        this.$refs.resourceTree.reload(item);
      });
    },
    ...
    //     this.$refs.resourceTree.setCheckedNodes(item);
    //   });
    // },
    ...
    treeCheckChange(obj, isCheck, isLeafCheck) {
      // console.log(obj)
    },
    treeCheck(checkedNode,checkedKey) {
      this.checkedKeys = checkedKey.checkedKeys;
      // 勾选展开所有子节点
    ...
    setIcon(data) {
      data.forEach(item => {
        ...
        if (
          ...
          if (this.checkedList.indexOf(cItem.id) !== -1 && this.vehicleStatus === 'on') {
            cItem.icon = 'rm-tree-vehicle';
          }
          if (this.checkedList.indexOf(cItem.id) !== -1 && this.vehicleStatus === 'off') {
            cItem.icon = 'rm-tree-vehicle onlineStop';
          }
          if (!cItem.online) {
            // cItem.icon = 'rm-tree-vehicle off';
            ...
            if (this.checkedList.indexOf(cItem.id) !== -1 && this.vehicleStatus === 'off') {
              cItem.icon = 'rm-tree-vehicle offlineStop';
            }
          }
        }
